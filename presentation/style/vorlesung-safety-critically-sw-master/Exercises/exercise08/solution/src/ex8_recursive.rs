...
impl <K: Ord, V> BST<K, V> {
    ...
    pub fn size(&self) -> usize {
        match self.node {
            None           => 0,
            Some(ref node) => 1 + node.left.size() + node.right.size()
        }
    }
    ...
    pub fn lookup(&self, key: K) -> Option<&V> {
        match self.node {
            None                             => None,
            Some(ref node) if key < node.key => node.left.lookup(key),
            Some(ref node) if key > node.key => node.right.lookup(key),
